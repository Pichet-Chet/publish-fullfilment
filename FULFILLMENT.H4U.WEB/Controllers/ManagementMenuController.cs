...
namespace FULFILLMENT_H4U.Controllers
{
    public class ManagementMenuController : Controller
    {
        ...
        public ManagementMenuController(ILogger<HomeController> logger, IConfiguration configuration, IWebHostEnvironment hostingEnvironment)
        {
            _logger = logger;
        ...
        public async Task<JsonResult> GET_GROUP([FromQuery] FilterModel param)
        {
            Response resp = new Response();
        ...
        public async Task<JsonResult> DETAIL(int id)
        {
            ...
            try
            {
                resp = await _repoCollection.LIST_GET_DETAIL(id);
        ...
        public async Task<JsonResult> INSERT([FromBody] SysMenuList model)
        {
            Response resp = new Response();
            ...
            try
            {
                if (model != null)
                {
                    ...
                    resp = await _repoCollection.LIST_INSERT(model);
                }
                else
                {
                    resp.status = false;
        ...
        public async Task<JsonResult> UPDATE([FromBody] SysMenuList model)
        {
            Response resp = new Response();
