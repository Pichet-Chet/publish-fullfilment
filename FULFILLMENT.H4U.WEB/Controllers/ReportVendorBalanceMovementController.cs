...
{
    public class ReportVendorBalanceMovementController : Controller
    {
        IMasterVendorRepository _repoCollection;

        public ReportVendorBalanceMovementController(IConfiguration configuration)
        {
        ...
        public async Task<JsonResult> GET_BALANCE_MOVEMENT([FromQuery] FilterModel param)
        {
            Response resp = new Response();

            try
            {
                resp = _repoCollection.GET_BALANCE_MOVEMENT(param).Result;
            }
            catch (Exception ex)
            {
                resp.status = false;
