...
{
    public class ReportStockMovementController : Controller
    {
        IReportStockRepository _repoCollection;

        public ReportStockMovementController(IConfiguration configuration)
        {
        ...
        public async Task<JsonResult> GET_STOCK_MOVE_MENT([FromQuery] FilterModel param)
        {
            Response resp = new Response();

            try
            {
                resp = _repoCollection.GET_STOCK_MOVE_MENT(param).Result;
            }
            catch (Exception ex)
            {
                resp.status = false;
