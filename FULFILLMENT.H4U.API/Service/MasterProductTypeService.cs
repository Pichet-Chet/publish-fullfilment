...
{
    public class MasterProductTypeService
    {
        ...
        public async Task<Response> GET_ALL(FilterModel param)
        {
            ...
            try
            {
                var queryable = await Task.Run(() => _dbContext.MasterProductsTypes.AsQueryable());

                if (!string.IsNullOrEmpty(param.name))
                {
                    queryable = queryable.Where(x => param.name.Contains(x.Value)).AsQueryable();
                }
        ...
        public async Task<Response> GET_DETAIL(int id)
        {
            ...
            try
            {
                var getData = await Task.Run(() => _dbContext.MasterProductsTypes.Where(x => x.Id == id).FirstOrDefault());
        ...
        public async Task<Response> INSERT(MasterProductsType param)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var checkDuplication = _dbContext.MasterProductsTypes.Where(x => x.Value == param.Value).FirstOrDefault();
                    ...
                    if (validate == true)
                    {
                        ...
                        await Task.Run(() => _dbContext.MasterProductsTypes.Add(param));
        ...
        public async Task<Response> UPDATE(MasterProductsType param)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var checkDuplication = _dbContext.MasterProductsTypes.Where(x => x.Value == param.Value && x.Id != param.Id).FirstOrDefault();
                    ...
                    if (validate == true)
                    {
                        var update = await Task.Run(() => _dbContext.MasterProductsTypes.Where(x => x.Id == param.Id).FirstOrDefault());
        ...
        public async Task<Response> DELETE(int id)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var delete = await Task.Run(() => _dbContext.MasterProductsTypes.Find(id));

                    if (delete != null)
                    {
                        _dbContext.MasterProductsTypes.Remove(delete);
