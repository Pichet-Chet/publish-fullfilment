...
namespace FULFILLMENT.H4U.API.Service
{
    public class MasterPlatformService
    {
        ...
        public async Task<Response> GET_ALL(FilterModel param)
        {
            ...
            try
            {
                var queryable = await Task.Run(() => _dbContext.MasterPlatforms.AsQueryable());
        ...
        public async Task<Response> GET_DETAIL(int id)
        {
            ...
            try
            {
                var getData = await Task.Run(() => _dbContext.MasterPlatforms.Where(x => x.Id == id).FirstOrDefault());
        ...
        public async Task<Response> INSERT(MasterPlatform param)
        {
            Response resp = new Response();
            ...
            try
            {
                using (_dbContext)
                {
                    var checkDuplication = _dbContext.MasterPlatforms.Where(x => x.Value == param.Value).FirstOrDefault();
                    ...
                    if (validate == true)
                    {
                        ...
                        await Task.Run(() => _dbContext.MasterPlatforms.Add(param));
        ...
        public async Task<Response> UPDATE(MasterPlatform param)
        {
            Response resp = new Response();
            ...
            try
            {
                using (_dbContext)
                {
                    var checkDuplication = _dbContext.MasterPlatforms.Where(x => x.Value == param.Value && x.Id != param.Id).FirstOrDefault();
                    ...
                    if (validate == true)
                    {
                        var update = await Task.Run(() => _dbContext.MasterPlatforms.Where(x => x.Id == param.Id).FirstOrDefault());
        ...
        public async Task<Response> DELETE(int id)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var delete = await Task.Run(() => _dbContext.MasterPlatforms.Find(id));

                    if (delete != null)
                    {
                        _dbContext.MasterPlatforms.Remove(delete);
